...
class VivitTubeletEmbeddings(nn.Module):
    ...
    def __init__(self, config):
        ...
        ############## Hard coded configs ########
        self.customConfig = {"num_tokens":1590,"DEEP":True,"VPT":True}
        ...
        if self.customConfig["VPT"]:
            self.num_patches += self.customConfig["num_tokens"]  
        ...
        num_tokens = self.customConfig["num_tokens"]
        # print(f"num tokens = {num_tokens} and deep = {deep} and layers = {config.num_hidden_layers}")
        if self.customConfig["VPT"]:
            print("Enabling VPT")
        ...
        ######For VPT-Deep##### done at encoder level
        if self.customConfig["DEEP"]:  # noqa
            print("Fine tuning method is DEEP")
            # logger.info("Fine tuning method is DEEP")

        #     total_d_layer = config.num_hidden_layers-1 #bcz for 1st layer we are including prompts for shallow VPT hence we are just using that.
        #     self.deep_prompt_embeddings = nn.Parameter(torch.zeros(
        #         total_d_layer, num_tokens, prompt_dim))
        #      # xavier_uniform initialization
        #     nn.init.uniform_(self.deep_prompt_embeddings.data, -val, val)
        # print("logger is being printed")
        # logger.info("logger is being printed")
    ...
    def incorporate_prompt(self, x):
        ...
        x = torch.cat((
                # x[:, :1, :],#cls because cls is added after adding positional embeddings so not requried here
    ...
    def forward(self, pixel_values, interpolate_pos_encoding: bool = False):
        ...
        if self.customConfig["VPT"]:
            x = self.incorporate_prompt(x)#prompted embeddings
...
class VivitEncoder(nn.Module):
    def __init__(self, config):
        ...
        ##############Custom config ###################
        self.customConfig = {"num_tokens":1590,"DEEP":True}
        ###############VPT DEEP###################
        if self.customConfig["DEEP"]:  # noqa
            # print("entered")
            num_tokens = self.customConfig["num_tokens"]
    ...
    def forward(
        ...
        for i, layer_module in enumerate(self.layer):
            ...
            # print(f"self.gradient_checkpointing = {self.gradient_checkpointing} and self.training = {self.training}")
            if self.gradient_checkpointing and self.training:
                ...
                if self.customConfig["DEEP"] and i!=0:
                    Batch = hidden_states.shape[0]
                    if i <= self.deep_prompt_embeddings.shape[0]:#only for deep
                    # deep_prompt_emb = self.prompt_dropout(self.prompt_proj(
                    #     self.deep_prompt_embeddings[i-1]).expand(B, -1, -1))
                        # print(f"layer no = {i} and Batch = {Batch}")
                        hidden_states = torch.cat((
                            hidden_states[:, :1, :],
                            self.deep_prompt_embeddings[i-1].expand(Batch, -1, -1),
                            hidden_states[:, (1+self.num_tokens):, :]
                        ), dim=1)
                # print(f"Hidden_states size before sending to attn layer = {hidden_states.shape}")
                layer_outputs = layer_module(hidden_states, layer_head_mask, output_attentions)
                # print(f"layer_outputs size after sending to attn layer = {len(layer_outputs)}")

            hidden_states = layer_outputs[0]
            # print(f"Hidden_states size after sending to attn layer = {hidden_states.shape}")
...
class VivitModel(VivitPreTrainedModel):
    ...
    def forward(
        ...
        sequence_output = encoder_outputs[0]#last layer outputs (b_s,embeddings,hiddendim)
...
class VivitForVideoClassification(VivitPreTrainedModel):
    def __init__(self, config):
        super().__init__(config)
        self.customConfig = {"transfer_type":"prompt"}#to avoid full fine-tuning
        transfer_type = self.customConfig["transfer_type"]
        ...
        total_layer = len(self.vivit.encoder.layer)
        print(f"Total number of layers = {total_layer}")
        if transfer_type == "partial-1":
            # tuned_params = [
            #     "transformer.encoder.layer.{}".format(i-1) for i in range(total_layer)]
            for k, p in self.vivit.named_parameters():
                if "encoder.layer.{}".format(total_layer - 1) not in k and "layernorm" not in k: # noqa
                    p.requires_grad = False
        elif transfer_type == "partial-2":
            for k, p in self.vivit.named_parameters():
                if "encoder.layer.{}".format(total_layer - 1) not in k and "encoder.layer.{}".format(total_layer - 2) not in k and "layernorm" not in k: # noqa
                    p.requires_grad = False

        elif transfer_type == "partial-4":
            for k, p in self.vivit.named_parameters():
                if "encoder.layer.{}".format(total_layer - 1) not in k and "encoder.layer.{}".format(total_layer - 2) not in k and "encoder.layer.{}".format(total_layer - 3) not in k and "encoder.layer.{}".format(total_layer - 4) not in k and "layernorm" not in k: # noqa
                    p.requires_grad = False
        ...
        #     for k, p in self.vivit.named_parameters():
        #         if "prompt" not in k and "embeddings.patch_embeddings.projection.weight" not in k  and "embeddings.patch_embeddings.projection.bias" not in k:
        #             p.requires_grad = False

        elif transfer_type == "prompt":
            for k, p in self.vivit.named_parameters():
                if "prompt" not in k:#add classfier as true for this
                    p.requires_grad = False
            ...
            nn.init.normal_(
                self.vivit.embeddings.cls_token,
                std=1e-6
            )
        ...
        # adapter
            ...
            raise "Incorrect transfer type"

################only Printing layers and their grads##########
        for k,p in self.vivit.named_parameters():
            if p.requires_grad:
                print(f"{k} requires grad = {p.requires_grad} and trainable params = {p.numel()}")
            else:
                print(f"{k} requires grad = {p.requires_grad}")

            

        print(f"classifier layer: ")
        for k,p in self.classifier.named_parameters():
            print(f"{k} requires grad = {p.requires_grad} and number of params = {p.numel()}")
        # if transfer_type != "prompt" and transfer_type != "end2end" and transfer_type != "cls+prompt": # cls_head is trained only for prompt and full finetuning
        #     for k,p in self.classifier.named_parameters():
        #         p.requires_grad = False
        #         print(f"{k} requires grad = {p.requires_grad}")
